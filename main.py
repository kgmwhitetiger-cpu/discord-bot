...
async def report_command(
    ...
    try:
        ...
        await interaction.response.send_message("신고가 성공적으로 접수되었습니다!", ephemeral=True)
...
async def list_command(interaction: discord.Interaction):
    if not reports_db:
        await interaction.response.send_message("현재 등록된 명단이 없습니다.", ephemeral=True)
        return
...
async def delete_command(interaction: discord.Interaction, 닉네임: str):
    ...
    if name in reports_db:
        del reports_db[name]
        save_data(reports_db)
        await interaction.response.send_message(f"✅ `{name}` 님의 신고 데이터가 삭제되었습니다.", ephemeral=True)
    else:
